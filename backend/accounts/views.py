...
def build_conversation_key(user_one_id: int, user_two_id: int) -> str:
    left_id, right_id = sorted([int(user_one_id), int(user_two_id)])
...
class MessageCreateView(APIView):
    ...
    def get_direct_message_members(self, request):
        active_department = get_active_department(request)
        if active_department is None:
            return Response([])
        ...
        for profile in sorted(profiles, key=profile_priority, reverse=True):
            ...
            conversation_key = build_conversation_key(request.user.id, profile.user_id)

            unread_count = 0
            last_message_at = None
            messages = Message.objects.filter(conversation=conversation_key)
            if messages.exists():
                unread_count = messages.filter(recipient=request.user.id, is_read=False).count()
    ...
    def get_conversation_messages(self, request, receiver_id: int):
        user_model = get_user_model()
        receiver = user_model.objects.filter(pk=receiver_id, is_active=True).first()
        if receiver is None:
        ...
        conversation_key = build_conversation_key(request.user.id, receiver.pk)
        messages = Message.objects.filter(conversation=conversation_key).order_by("created_at", "id")
        unread_messages = messages.filter(recipient=request.user.id, is_read=False)
    ...
    def get(self, request):
        ...
        if receiver_id:
            try:
                return self.get_conversation_messages(request, int(receiver_id))
            except ValueError:
                return Response({"detail": "user_id must be a valid integer."}, status=status.HTTP_400_BAD_REQUEST)

        return self.get_direct_message_members(request)
    ...
    def post(self, request):
        ...
        user_model = get_user_model()
        recipient = user_model.objects.filter(pk=serializer.validated_data["recipient_id"], is_active=True).first()
        if recipient is None:
            return Response({"detail": "Recipient not found."}, status=status.HTTP_404_NOT_FOUND)

        active_department = get_active_department(request)
        recipient_profile = get_auth_profile(recipient)
        if active_department is None:
            return Response({"detail": "Department not found."}, status=status.HTTP_400_BAD_REQUEST)
        if recipient.pk != request.user.pk and (
            recipient_profile is None or recipient_profile.home_department_id != active_department.id
        ):
            return Response({"detail": "This team member is not in the current department."}, status=status.HTTP_400_BAD_REQUEST)

        conversation_key = build_conversation_key(request.user.id, recipient.pk)
